...
# -------------------------
# WIDGET / EMBED MODE
# Load this app inside an iframe with ?embed=widget on the IRD Grenada
# website (see the floating-bubble launcher snippet: ird_chat_widget.html)
# to render it as a compact corner chat widget instead of a full page.
# -------------------------
TRUE_IFRAME_EMBED = st.query_params.get("embed") == "widget"
if "widget_preview" not in st.session_state:
    st.session_state.widget_preview = TRUE_IFRAME_EMBED
WIDGET_MODE = TRUE_IFRAME_EMBED or st.session_state.widget_preview

# COMPACT_FLOATING = the in-app "Compact" setting (Chat Settings / the
# preview toggle) rendering TESSA as a floating bottom-right bubble that
# sits on top of the rest of the app - as opposed to TRUE_IFRAME_EMBED,
# where the whole browser tab/iframe IS the widget (e.g. embedded on the
# public IRD Grenada website) and therefore doesn't need to "float" over
# anything. Both share the same compact layout (smaller chat pane,
# collapsed menus) but only COMPACT_FLOATING gets the fixed-position CSS
# treatment and the collapse-to-avatar behavior.
COMPACT_FLOATING = st.session_state.widget_preview and not TRUE_IFRAME_EMBED

if "compact_panel_open" not in st.session_state:
    st.session_state.compact_panel_open = True
if "compact_bubble_dismissed" not in st.session_state:
    st.session_state.compact_bubble_dismissed = False

ASSETS_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "assets")

# ROOT CAUSE (avatar not displaying): the app only ever looked for a
# single exact filename ("tessa_avatar.png"). If the real asset was
# committed under a different name/extension (tessa_avatar.jpg,
# TESSA_Avatar.PNG, avatar.png, etc.) or the assets/ folder was missing
# entirely from what got pushed to GitHub -> Render, os.path.exists()
# silently failed and every avatar_html block quietly rendered nothing.
# Fix: search a small set of likely names/extensions instead of assuming
# one exact filename, and resolve+cache the result once per process
# instead of re-reading the file from disk on every single render call.
_AVATAR_CANDIDATES = [
    "tessa_avatar.png", "tessa_avatar.jpg", "tessa_avatar.jpeg",
    "tessa_avatar.webp", "avatar.png", "avatar.jpg",
    "TESSA_avatar.png", "tessa-avatar.png",
]


def _resolve_tessa_avatar_path():
    """Return the first existing avatar asset path, or None if the
    project genuinely doesn't contain one yet (case-insensitive match
    against whatever is actually in assets/)."""
    if not os.path.isdir(ASSETS_DIR):
        return None
    try:
        existing = {f.lower(): f for f in os.listdir(ASSETS_DIR)}
    except OSError:
        return None
    for candidate in _AVATAR_CANDIDATES:
        real_name = existing.get(candidate.lower())
        if real_name:
            return os.path.join(ASSETS_DIR, real_name)
    return None


TESSA_AVATAR = _resolve_tessa_avatar_path()
TESSA_AVATAR_AVAILABLE = TESSA_AVATAR is not None

# NOTE FOR DEPLOYMENT: as of this fix, no avatar file was found under
# assets/ at all in the repository this app was inspected in. Drop the
# real TESSA avatar image into assets/ using ANY of the filenames in
# _AVATAR_CANDIDATES above (e.g. assets/tessa_avatar.png) and it will be
# picked up automatically everywhere the avatar is used - chat bubbles,
# the header, and the floating widget - with no further UI code changes.
# Until then the app falls back to a clean circular "T" monogram (see
# get_avatar_b64() below) instead of a broken image / blank space.

_FALLBACK_AVATAR_SVG = """
<svg xmlns="http://www.w3.org/2000/svg" width="120" height="120" viewBox="0 0 120 120">
  <circle cx="60" cy="60" r="60" fill="#0e5fa8"/>
  <circle cx="60" cy="60" r="60" fill="none" stroke="#f4d216" stroke-width="4"/>
  <text x="60" y="78" font-family="Poppins, Segoe UI, sans-serif" font-size="56"
...
def get_avatar_b64():
    """Base64 image data (without the data: prefix) for TESSA's avatar,
    read from disk once and cached for the life of the process - never
    re-read on every chat bubble render. Falls back to a generated SVG
    monogram (also base64-encoded) if no real asset is present, so the
    UI always has something valid to show instead of a broken <img>."""
    cache_key = TESSA_AVATAR or "__fallback__"
    if cache_key in _avatar_b64_cache:
        return _avatar_b64_cache[cache_key]

    if TESSA_AVATAR_AVAILABLE:
        try:
            with open(TESSA_AVATAR, "rb") as f:
                encoded = base64.b64encode(f.read()).decode()
        except OSError:
            encoded = base64.b64encode(_FALLBACK_AVATAR_SVG.encode("utf-8")).decode()
    else:
        encoded = base64.b64encode(_FALLBACK_AVATAR_SVG.encode("utf-8")).decode()

    _avatar_b64_cache[cache_key] = encoded
    return encoded


def get_avatar_mime():
    """MIME type matching whatever get_avatar_b64() returned."""
    if TESSA_AVATAR_AVAILABLE:
        ext = os.path.splitext(TESSA_AVATAR)[1].lower()
        return {
...
# Tessa's own chat replies are translated dynamically by the model (see
# LANGUAGES above / SOURCING rule). The surrounding app chrome (sidebar
# labels, page headers) is static text, so it's translated here for the
# core, most-visited labels. Anything not listed below simply stays in
# English rather than showing a broken/missing translation.
# -------------------------
UI_STRINGS = {
    "My Profile / Sign In": {
        "Grenadian Creole (Patois)": "Mi Profile / Sign In",
        "French": "Mon profil / Connexion",
        "Spanish": "Mi perfil / Iniciar sesión",
    },
    "Your Taxpayer ID": {
        "Grenadian Creole (Patois)": "Yuh Taxpayer ID",
        "French": "Votre identifiant contribuable",
        "Spanish": "Su ID de contribuyente",
    },
    "My Territory (Parish)": {
        "Grenadian Creole (Patois)": "Mi Territory (Parish)",
        "French": "Ma paroisse / territoire",
        "Spanish": "Mi territorio (parroquia)",
    },
    "I am a...": {
        "Grenadian Creole (Patois)": "Ah is a...",
        "French": "Je suis un(e)...",
        "Spanish": "Soy un(a)...",
    },
    "Are you a business owner?": {
        "Grenadian Creole (Patois)": "Yuh is a business owner?",
        "French": "Êtes-vous propriétaire d'une entreprise ?",
        "Spanish": "¿Es usted propietario de un negocio?",
    },
    "Language": {
        "Grenadian Creole (Patois)": "Language",
        "French": "Langue",
        "Spanish": "Idioma",
    },
    "Tone": {
        "Grenadian Creole (Patois)": "Tone",
        "French": "Ton",
        "Spanish": "Tono",
    },
    "Quick Contact": {
        "Grenadian Creole (Patois)": "Quick Contact",
        "French": "Contact rapide",
        "Spanish": "Contacto rápido",
    },
    "Frequently Asked Questions": {
        "Grenadian Creole (Patois)": "Question People Does Ask a Lot",
        "French": "Foire aux questions",
        "Spanish": "Preguntas frecuentes",
    },
    "Tax Terms Glossary": {
        "Grenadian Creole (Patois)": "Tax Word Dem Explain",
        "French": "Glossaire des termes fiscaux",
        "Spanish": "Glosario de términos fiscales",
    },
    "IRD Offices Across Grenada": {
        "Grenadian Creole (Patois)": "IRD Office Dem All Over Grenada",
        "French": "Bureaux de l'IRD à travers la Grenade",
        "Spanish": "Oficinas del IRD en Granada",
    },
    "Tax Deadline Calendar": {
        "Grenadian Creole (Patois)": "Tax Deadline Calendar",
        "French": "Calendrier des échéances fiscales",
        "Spanish": "Calendario de vencimientos fiscales",
    },
    "Report an Issue": {
        "Grenadian Creole (Patois)": "Report Sinting That Happen",
        "French": "Signaler un problème",
        "Spanish": "Reportar un problema",
    },
}


def tr(key):
    """Look up a UI-chrome string in the current language; falls back to
    the original English key if no translation is defined."""
...
def safe_secret(key, default=""):
    """st.secrets.get() raises StreamlitSecretNotFoundError if NO
    secrets.toml file exists at all (e.g. on Render, where config is via
    real env vars instead) - this never raises, just falls through."""
    try:
...
MODEL_NAME = os.environ.get("GEMINI_MODEL_NAME") or safe_secret("GEMINI_MODEL_NAME", "gemini-3.1-flash-lite")

# -------------------------
# SYSTEM HEALTH (drives the header status dot + banner)
# "online"   -> last live API call succeeded (or none attempted yet)
# "degraded" -> last call failed for a non-transient, non-user-fixable
#               reason (bad/expired key, permission, model unavailable) -
#               retrying immediately won't help, so we say so plainly
#               instead of repeating a raw error on every message.
# -------------------------
if "system_health" not in st.session_state:
    st.session_state.system_health = "online"


# -------------------------
# ERROR CLASSIFICATION + PROFESSIONAL ERROR MESSAGES
# The raw exceptions the Gemini SDK raises (e.g. a full googleapis JSON
# error blob for an invalid key) are meaningless and unprofessional to
# show a taxpayer. This layer:
#   1. Classifies the failure (bad key, quota, temporarily unavailable...)
#   2. Logs the FULL technical detail to the staff bug log (Admin
#      Dashboard -> Bugs) so nothing is lost for debugging
#   3. Returns a short, clean, user-safe sentence instead
# -------------------------
def _error_kind(exc):
    text = str(exc).lower()
    ...
    if "resource_exhausted" in text or "quota" in text or "429" in text:
        return "quota"
    if ("unavailable" in text or "503" in text or "deadline" in text
...
# Failures where an immediate retry has no chance of succeeding - these
# need a human (staff) to fix configuration, not another API call.
NON_TRANSIENT_ERROR_KINDS = {"invalid_key", "permission", "bad_model"}
# Failures worth automatically retrying with backoff before giving up.
RETRYABLE_ERROR_KINDS = {"quota", "unavailable"}

FRIENDLY_ERROR_TEXT = {
    "invalid_key": "TESSA's connection to its AI service isn't configured correctly on our end",
    "permission": "TESSA's AI service access is currently restricted on our end",
    "quota": "TESSA is getting a lot of requests right now and briefly hit its usage limit",
    "unavailable": "TESSA's AI service is temporarily unavailable",
    "bad_model": "TESSA's AI service is currently being reconfigured on our end",
    "unknown": "TESSA ran into an unexpected problem reaching the AI service",
}


def log_technical_error(where, exc):
    """Record the raw exception for staff (never shown to the taxpayer)."""
    try:
        save_bug_report(
            "system", where, "High",
            f"[auto-logged] {_error_kind(exc)}: {exc}"[:500],
        )
    except Exception:
        pass


def friendly_api_error(exc, where="Chat"):
    """Classify + log a live-API failure, update the header health status,
    and return one clean, professional sentence to show the user."""
    kind = _error_kind(exc)
    log_technical_error(where, exc)
    if kind in NON_TRANSIENT_ERROR_KINDS:
        st.session_state.system_health = "degraded"
    return (
        f"⚠️ {FRIENDLY_ERROR_TEXT.get(kind, FRIENDLY_ERROR_TEXT['unknown'])}. "
        "Please try again in a moment, check the ❓ FAQs tab, or contact "
        "the Inland Revenue Division for assistance."
    )


def call_with_retries(fn, *args, retries=2, base_delay=1.0, **kwargs):
    """Run fn(*args, **kwargs), auto-retrying with exponential backoff on
    transient errors (rate limit / temporary outage) only. Non-transient
    errors (bad key, permission, missing model) fail immediately instead
    of burning retries on something that can't self-resolve."""
    last_exc = None
    for attempt in range(retries + 1):
        try:
            ...
            last_exc = exc
            if _error_kind(exc) not in RETRYABLE_ERROR_KINDS or attempt == retries:
                raise
...
# -------------------------
# ADMIN / STAFF ACCESS
# A single shared staff password gates the Admin Dashboard. Set it via
# .streamlit/secrets.toml (ADMIN_PASSWORD = "...") or an ADMIN_PASSWORD
# env var. If neither is set, the dashboard stays locked and shows a
# setup notice instead of silently exposing taxpayer data.
# -------------------------
ADMIN_PASSWORD = safe_secret("ADMIN_PASSWORD", "")
...
# -------------------------
# USER SIGN-IN / PROFILE (gives each user a unique ID that IRD staff can
# use in the Admin Dashboard to pull up their info - name, taxpayer type,
# whether they're a business owner, and their territory/parish)
# -------------------------
import uuid

USER_COLUMNS = [
    "user_id", "created_at", "updated_at", "name", "email",
    "taxpayer_type", "is_business_owner", "territory",
]


def load_users_df():
    return _load_table_df("users", USER_COLUMNS)
...
def upsert_user_profile(user_id, name, email, taxpayer_type, is_business_owner, territory):
    """Sign-in profiles are append-only (like every other log table here),
    so 'update' means writing a new row with the same user_id - the Admin
    Dashboard shows the most recent row per user_id as current."""
    now = datetime.now().isoformat(timespec="seconds")
    existing = find_user_by_id(user_id)
    return _append_table_row("users", USER_COLUMNS, {
        "user_id": user_id,
        ...
        "is_business_owner": "Yes" if is_business_owner else "No",
        "territory": territory,
    })


# -------------------------
# SECURITY / INCIDENT REPORTS (hacked account, suspicious activity, lost
# ID, etc.) - visible to staff in the Admin Dashboard.
# -------------------------
SECURITY_REPORT_COLUMNS = [
    "timestamp", "user_id", "user_name", "contact_info",
    "issue_type", "description", "status",
]
SECURITY_ISSUE_TYPES = [
    "My G-TAX/portal account was hacked",
    "Suspicious activity on my account",
    "I lost my TIN card / Tax Clearance Certificate",
    "I suspect a scam/phishing message claiming to be from IRD",
    "Identity theft / someone filed using my information",
    "Other security concern",
]


def load_security_reports_df():
    return _load_table_df("security_reports", SECURITY_REPORT_COLUMNS)


def save_security_report(user_id, user_name, contact_info, issue_type, description):
    return _append_table_row("security_reports", SECURITY_REPORT_COLUMNS, {
        "timestamp": datetime.now().isoformat(timespec="seconds"),
        "user_id": user_id or "",
        "user_name": user_name or "Anonymous",
        "contact_info": contact_info,
...
# -------------------------
def build_system_instruction(user_name=None, memory=None, language="English", tone="Friendly (default)", profile=None):
    base = """
...
SOURCING & CITATION RULE: For every factual answer (rates, deadlines, rules,
procedures, requirements), briefly state where the information comes from
and, whenever you have it (e.g. from a web search), include the direct
official link. Use a short "Source:" line at the end of your answer, e.g.
"Source: IRD Grenada - tax.gov.gd" or "Source: [page title](https://...)".
If you're answering from your own general/offline knowledge rather than a
live lookup, say so plainly (e.g. "Source: general IRD Grenada guidance -
please confirm the current figure via 🔎 Deep Search") instead of inventing
a link. Never fabricate a URL - only include a link you actually retrieved.
...
"""
    ...
    if profile:
        ...
        if profile.get("taxpayer_type"):
            details.append(f"taxpayer type: {profile['taxpayer_type']}")
        if profile.get("is_business_owner"):
            details.append(f"business owner: {profile['is_business_owner']}")
        if profile.get("territory"):
            details.append(f"territory/parish: {profile['territory']}")
        if details:
            base += (
                "\nKnown profile info for this user (use to tailor guidance, "
                "e.g. pointing to their nearest office or business-specific "
                "rules, but don't recite it back mechanically): "
                + "; ".join(details) + "\n"
            )
...
def get_tax_news(query="latest Grenada Inland Revenue Division tax news, deadlines, and updates"):
    ...
    try:
        ...
        log_technical_error("Tax News", e)
        return (
            "⚠️ I couldn't fetch live news right now. Please check the "
...
def deep_research(query):
    ...
    try:
        ...
        return friendly_api_error(e, where="Deep Search"), False


# -------------------------
# TEXT-TO-SPEECH "LISTEN" BUTTON
# Primary voice: ElevenLabs (feminine, warm, Caribbean-accented voice).
# Fallback voice: the browser's built-in speechSynthesis (original
# behavior) - used automatically if ElevenLabs isn't configured, or if a
# live call to it fails, so Listen never just stops working.
# -------------------------
# -------------------------
...
# Both values come ONLY from Streamlit secrets or environment variables -
# never hardcoded, so nothing sensitive ends up in source control. Missing
# either one simply disables ElevenLabs and the app falls back to the
# original browser voice; it does NOT crash the app (voice is an
# enhancement, not a hard dependency).
ELEVENLABS_API_KEY = safe_secret("ELEVENLABS_API_KEY", "")
if not ELEVENLABS_API_KEY:
    ELEVENLABS_API_KEY = os.environ.get("ELEVENLABS_API_KEY", "")

ELEVENLABS_VOICE_ID = safe_secret("ELEVENLABS_VOICE_ID", "")
if not ELEVENLABS_VOICE_ID:
    ELEVENLABS_VOICE_ID = os.environ.get("ELEVENLABS_VOICE_ID", "")

# eleven_multilingual_v2 gives the most natural, least "robotic" delivery
# for a warm customer-service voice. Overridable via env var if a faster/
# cheaper model is preferred later.
ELEVENLABS_MODEL_ID = os.environ.get("ELEVENLABS_MODEL_ID", "eleven_multilingual_v2")
ELEVENLABS_ENABLED = bool(ELEVENLABS_API_KEY and ELEVENLABS_VOICE_ID)

# Tuned for natural warmth rather than a flat, over-stable, "read-aloud"
# tone or an unstably over-acted one:
#  - stability: moderate (a bit below default) keeps natural pitch/warmth
#    instead of a flat monotone, without going unstable.
#  - similarity_boost: high, to stay faithful to the chosen Caribbean voice.
#  - style: a little exaggeration for natural conversational expressiveness,
#    kept low so it stays professional rather than dramatic.
...
# Per-session in-memory cache only (no local files) - safe on Render's
# ephemeral filesystem and avoids re-billing ElevenLabs if the same
# message's Listen button is clicked more than once in a session.
if "tts_audio_cache" not in st.session_state:
    st.session_state.tts_audio_cache = {}


def elevenlabs_tts(text):
    """Call the ElevenLabs TTS API for TESSA's voice.
    Returns (audio_bytes, True) on success, or (error_message, False)."""
    if not ELEVENLABS_ENABLED:
        return "ElevenLabs is not configured (missing API key or voice ID).", False
    ...
    cache_key = hashlib.sha256(
        f"{ELEVENLABS_VOICE_ID}:{ELEVENLABS_MODEL_ID}:{clean_text}".encode("utf-8")
    ).hexdigest()
    cached = st.session_state.tts_audio_cache.get(cache_key)
    if cached:
        return cached, True
    ...
    try:
        resp = requests.post(
            ...
            json={
                # ElevenLabs caps request length; TESSA's replies are short
                # enough in practice, but this guards against an edge case.
        ...
        return f"ElevenLabs request failed: {e}", False

    if resp.status_code == 200 and resp.content:
        st.session_state.tts_audio_cache[cache_key] = resp.content
        # Cap the in-memory cache so a long session doesn't grow unbounded.
        if len(st.session_state.tts_audio_cache) > 60:
            st.session_state.tts_audio_cache.pop(
                next(iter(st.session_state.tts_audio_cache)), None
            )
        return resp.content, True

    if resp.status_code == 401:
        detail = "ElevenLabs rejected the API key (401 - invalid or missing ELEVENLABS_API_KEY)."
    elif resp.status_code == 422:
        detail = f"ElevenLabs rejected the request (422 - check ELEVENLABS_VOICE_ID is valid). {resp.text[:200]}"
    elif resp.status_code == 429:
        detail = "ElevenLabs quota/rate limit reached (429)."
    else:
...
def play_audio_autoplay(audio_bytes):
    """Embed a self-playing (but still user-controllable) HTML5 audio
    player. Runs client-side, so it plays immediately in the same
    Streamlit rerun the Listen button click triggered - one click, not two."""
    b64_audio = base64.b64encode(audio_bytes).decode()
    components.html(
        f"""
        <audio autoplay controls style="width:100%;height:32px;margin-top:4px;">
...
def _speak_button_browser(text, button_key):
    """Original client-side speechSynthesis fallback (used automatically
    when ElevenLabs isn't configured or a live call to it fails)."""
...
def speak_button(text, button_key):
    """TESSA's 'Listen' feature. Uses TESSA's ElevenLabs voice (feminine,
    warm, Caribbean-accented) when ELEVENLABS_API_KEY / ELEVENLABS_VOICE_ID
    are configured; otherwise transparently falls back to the browser's
    built-in voice (original behavior) - so this button always works."""
    if not ELEVENLABS_ENABLED:
        _speak_button_browser(text, button_key)
        return

    if st.button("🔊 Listen", key=f"el_listen_{button_key}"):
        with st.spinner("TESSA is preparing her voice..."):
            audio_or_error, ok = elevenlabs_tts(text)
        if ok:
            play_audio_autoplay(audio_or_error)
        else:
            log_technical_error("ElevenLabs TTS", audio_or_error)
            st.caption("⚠️ TESSA's voice service is briefly unavailable - using your device's voice instead.")
            _speak_button_browser(text, button_key)
...
def bubble_html(role, content, avatar_b64=None, avatar_mime="image/png"):
    is_user = role == "user"
    row_class = "user-row" if is_user else ""
    bubble_class = "user-bubble" if is_user else "assistant-bubble"
    avatar_html = ""
    if not is_user and avatar_b64:
        avatar_html = f'<img class="chat-avatar" src="data:{avatar_mime};base64,{avatar_b64}" />'
...
def render_bubble(role, content, avatar_b64=None, avatar_mime="image/png"):
    st.markdown(bubble_html(role, content, avatar_b64, avatar_mime), unsafe_allow_html=True)


def typing_indicator_html(avatar_b64=None, avatar_mime="image/png"):
    avatar_html = f'<img class="chat-avatar" src="data:{avatar_mime};base64,{avatar_b64}" />' if avatar_b64 else ""
    return (
...
# -------------------------
# CATEGORIES for the FAQ and Glossary tabs, so related questions/terms are
# grouped instead of one long alphabetical list. Assignment is keyword-based
# (checked against each question/term), so new entries auto-sort without
# needing to hand-tag every one of the 50+ items above.
# -------------------------
FAQ_CATEGORY_KEYWORDS = {
    "📝 Registration & TIN": ["tin", "register", "registration", "caipo", "sign up"],
    "💰 Payments & GCT": ["gct", "pay", "payment", "vat", "consumption tax", "refund"],
    "📄 Filing & Returns": ["file", "filing", "return", "deadline", "due date", "extension"],
    "🏠 Property Tax": ["property tax", "property", "land"],
    "🏢 Business Taxes": ["business", "employer", "payroll", "company", "self-employed"],
    "✅ Clearance & Compliance": ["clearance", "audit", "compliance", "penalty", "arrears", "objection", "appeal"],
    "💻 Online / G-TAX Portal": ["portal", "g-tax", "online", "e-service", "password", "login", "account locked"],
    "🏢 Offices & Contact": ["office", "contact", "hours", "location", "district"],
}
GLOSSARY_CATEGORY_KEYWORDS = FAQ_CATEGORY_KEYWORDS
CATEGORY_FALLBACK = "📌 General"


def categorize_items(items_dict, category_keywords):
    """Bucket a {question/term: answer/definition} dict into named
    categories using simple keyword matching against both the key and
    value text. Returns an ordered dict of category -> {key: value}."""
    buckets = {cat: {} for cat in category_keywords}
    buckets[CATEGORY_FALLBACK] = {}
    for key, value in items_dict.items():
        ...
        for cat, keywords in category_keywords.items():
            if any(kw in haystack for kw in keywords):
                ...
                break
        if not placed:
            buckets[CATEGORY_FALLBACK][key] = value
    # Drop empty categories so the tabs shown always have content.
    return {cat: items for cat, items in buckets.items() if items}


# -------------------------
# TAX DEADLINE CALENDAR
# General recurring deadline TYPES (not year-specific dates, which change -
# always confirm the current year's exact dates via 🔑 Key Info at a Glance
# or 🔎 Deep Search, both of which search the live official IRD site).
# -------------------------
TAX_DEADLINE_CALENDAR = [
    {"period": "Monthly", "item": "GCT (General Consumption Tax) Return & Payment", "typical_due": "By the 20th of the following month", "who": "GCT-registered businesses"},
    {"period": "Monthly", "item": "PAYE (Pay As You Earn) Remittance", "typical_due": "By the 15th of the following month", "who": "Employers"},
    {"period": "Annually", "item": "Individual Income Tax Return", "typical_due": "By March 31 (confirm current year's date)", "who": "Individual taxpayers with filing obligations"},
    {"period": "Annually", "item": "Corporate/Business Income Tax Return", "typical_due": "Confirm current year's date (varies by financial year-end)", "who": "Businesses & companies"},
    {"period": "Annually", "item": "Property Tax Payment", "typical_due": "Confirm current year's date", "who": "Property owners"},
    {"period": "Annually", "item": "Annual Business/Trade Licence Renewal", "typical_due": "Confirm current year's date", "who": "Licensed business owners"},
    {"period": "As needed", "item": "Tax Clearance Certificate Renewal", "typical_due": "Before the current certificate expires (typically valid 3-6 months)", "who": "Anyone requiring a valid clearance"},
]
...
OFFICES = [
    {
        ...
        "parish": "St. George",
        "lat": 12.0561, "lng": -61.7488,
    },
    {"name": "Sauteurs, St. Patrick", "location": "District Revenue Office, Sauteurs", "hours": "Monday – Friday", "phone": "+1 (473) 442-9324", "email": None, "parish": "St. Patrick", "lat": 12.1897, "lng": -61.6294},
    {"name": "Grenville, St. Andrew", "location": "District Revenue Office, Grenville", "hours": "Monday – Friday", "phone": "+1 (473) 442-7446 / 6904", "email": None, "parish": "St. Andrew", "lat": 12.1189, "lng": -61.6142},
    {"name": "Gouyave, St. John", "location": "District Revenue Office, Gouyave", "hours": "Monday – Friday", "phone": "+1 (473) 444-8231", "email": None, "parish": "St. John", "lat": 12.1633, "lng": -61.7364},
    {"name": "Victoria, St. Mark", "location": "District Revenue Office, Victoria", "hours": "Monday – Friday", "phone": "+1 (473) 444-8425", "email": None, "parish": "St. Mark", "lat": 12.1858, "lng": -61.6994},
    {"name": "St. David", "location": "District Revenue Office, St. David", "hours": "Monday – Friday", "phone": "+1 (473) 444-6243", "email": None, "parish": "St. David", "lat": 12.0139, "lng": -61.6636},
    {"name": "Carriacou", "location": "District Revenue Office, Hillsborough, Carriacou", "hours": "Monday – Friday", "phone": "+1 (473) 443-7388", "email": None, "parish": "Carriacou & Petite Martinique", "lat": 12.4747, "lng": -61.4497},
]

# All Grenada territories/parishes a user can pick as "my territory" -
# St. George's parish maps to the Main Office; every other parish maps to
# its own District Revenue Office above.
GRENADA_PARISHES = [o["parish"] for o in OFFICES]


def find_office_for_parish(parish):
    """Return the office record serving a given parish/territory."""
    for office in OFFICES:
        if office["parish"] == parish:
            return office
    return OFFICES[0]


def office_maps_embed_url(office):
    """No API key required - Google Maps supports a plain search-query
    embed. Falls back gracefully to a text address if maps can't load."""
    from urllib.parse import quote
    query = quote(f"{office['name']} IRD Grenada {office['location']}")
    return f"https://www.google.com/maps?q={query}&output=embed"


def office_maps_link(office):
    from urllib.parse import quote
    query = quote(f"{office['name']} IRD Grenada {office['location']}")
...
def build_form_guide_pdf(doc_name, steps):
    """Printable step-by-step guide for one form, as a real PDF via
    fpdf2. Returns (bytes_or_None, success_bool) - falls back to plain
    text if fpdf2 isn't installed, same pattern as the readiness PDF."""
    try:
        from fpdf import FPDF
        ...
        pdf.multi_cell(0, 9, f"IRD Grenada - {doc_name}")
        pdf.set_font("Helvetica", "", 9)
        pdf.cell(0, 6, f"Generated {datetime.now().strftime('%B %d, %Y')} by TESSA", ln=True)
        pdf.ln(3)
        pdf.set_font("Helvetica", "B", 12)
        pdf.cell(0, 8, "Step-by-Step Guide", ln=True)
        pdf.set_font("Helvetica", "", 10)
...
def build_form_guide_text(doc_name, steps):
    lines = [f"IRD Grenada - {doc_name}", f"Generated {datetime.now().strftime('%B %d, %Y')} by TESSA", ""]
    for i, step in enumerate(steps, start=1):
...
# -------------------------
RATE_LIMIT_MAX_PER_MINUTE = int(os.environ.get("RATE_LIMIT_MAX_PER_MINUTE", "90"))
...
def check_rate_limit():
    ...
    1. PER-SESSION cooldown (in-memory, session_state): stops a single
       browser tab from firing more than one model request within a short
       window - this is what protects against double-clicks, rapid-fire
       Enter presses, and any accidental duplicate submission, and it is
       the actual fix for "usage limit hit during normal testing": a lone
       tester was previously sharing the SAME tiny global counter as every
       other feature (chat, file upload, voice notes, news lookups, tax
       bracket lookups...), so a handful of clicks in one tab could exhaust
       the whole deployment's budget for everyone.
    2. GLOBAL (all-sessions) ceiling backed by SQLite, sized for real
       production traffic across every taxpayer hitting this deployment,
       not just one tab. This is deliberately generous - its job is to
       absorb genuine abuse/bursts, not to gate normal single-user testing.

    Returns True if the request is allowed, False if a limit is hit."""
    now = time.time()

    # --- Layer 1: per-session cooldown ---
    last_call = st.session_state.get("_last_model_call_ts", 0)
    if now - last_call < PER_SESSION_MIN_INTERVAL_SECONDS:
        return False
    st.session_state["_last_model_call_ts"] = now

    # --- Layer 2: global ceiling ---
    with _db_lock:
        conn = _get_db_connection()
        try:
            cur = conn.cursor()
...
# Minimum gap (seconds) between two model-hitting actions from the SAME
# browser session. Blocks accidental double-submits without being
# noticeable to a real user typing normal messages.
PER_SESSION_MIN_INTERVAL_SECONDS = 2.0
...
if WIDGET_MODE:
    st.markdown(
        ...
        .tessa-header { padding: 8px 12px; border-radius: 10px; margin-bottom: 6px; }
        .tessa-header img { width: 34px; height: 34px; }
        .tessa-header h1 { font-size: 17px !important; }
        .tessa-header p { font-size: 10px !important; }
        div[data-testid="stCaptionContainer"] { font-size: 10px; }
        .chat-scroll-pane { max-height: 44vh; }
        div.stButton > button, .stDownloadButton > button { padding: 4px 8px; font-size: 12px; }
        </style>
        """
        ),
        unsafe_allow_html=True,
...
if COMPACT_FLOATING:
    # Turns the compact layout into an actual floating bottom-right
    # widget instead of just shrinking the whole page in place. Anchors
    # on a small invisible marker div rendered as the first element of
    # the block we want to float, using the CSS :has() selector - no
    # JS required, so it survives every Streamlit rerun automatically
    # and needs no client/server state bridge.
    st.markdown(
        textwrap.dedent(
            """
        <style>
        div[data-testid="stVerticalBlockBorderWrapper"]:has(> div > div > div.tessa-float-marker),
        div[data-testid="stVerticalBlock"]:has(> div > div.tessa-float-marker) {
            ...
            background: #ffffff;
            border-radius: 16px;
            box-shadow: 0 8px 32px rgba(6,20,43,0.28);
            padding: 12px 14px 14px 14px;
            z-index: 99999;
            /* Respect iPad/iOS safe areas so it never sits under the
        ...
        .tessa-float-avatar-btn div.stButton > button {
            width: 62px; height: 62px; border-radius: 50%;
            padding: 0; font-size: 26px; line-height: 1;
            box-shadow: 0 6px 18px rgba(6,20,43,0.35);
            border: 2px solid #f4d216;
            background: linear-gradient(135deg, #0e5fa8, #06142b);
            color: #ffffff !important;
        }
        .tessa-float-avatar-wrap {
            position: fixed !important;
            bottom: 18px;
            right: 18px;
            z-index: 99999;
        ...
        .tessa-greeting-bubble {
            ...
            border-radius: 14px;
            padding: 12px 14px;
            box-shadow: 0 6px 20px rgba(6,20,43,0.25);
            font-size: 13.5px;
            color: #10182b;
            z-index: 99998;
...
if "ui_text_cache" not in st.session_state:
    st.session_state.ui_text_cache = {}

# -------------------------
# SIDEBAR
# -------------------------
def render_nav_controls():
    if TESSA_AVATAR_AVAILABLE:
        st.image(TESSA_AVATAR, width=110)
    else:
        st.markdown(
            f'<img src="data:{get_avatar_mime()};base64,{get_avatar_b64()}" '
            f'width="110" style="border-radius:50%;" />',
            unsafe_allow_html=True,
        )
    ...
    # ---------------------------------------------------------------
    # SIGN-IN / PROFILE: gives every user a unique Taxpayer ID (shown to
    # them) that IRD staff can search for in 🛠️ Admin Dashboard -> 👤 Users
    # to pull up their taxpayer type, business-owner status, and territory.
    # ---------------------------------------------------------------
    st.markdown("---")
    st.write(f"### 🪪 {tr('My Profile / Sign In')}")
    with st.expander("Sign in / update my profile", expanded=not st.session_state.user_id):
        profile_email = st.text_input(
            "Email (optional)", value=(st.session_state.user_profile or {}).get("email", ""),
            placeholder="you@email.com", key="profile_email_input",
        )
        profile_taxpayer_type = st.selectbox(
            tr("I am a..."), USER_TYPE_OPTIONS,
            index=USER_TYPE_OPTIONS.index((st.session_state.user_profile or {}).get("taxpayer_type", USER_TYPE_OPTIONS[0]))
            if (st.session_state.user_profile or {}).get("taxpayer_type") in USER_TYPE_OPTIONS else 0,
            key="profile_taxpayer_type_input",
        )
        profile_is_business_owner = st.checkbox(
            tr("Are you a business owner?"),
            value=(st.session_state.user_profile or {}).get("is_business_owner", "No") == "Yes",
            key="profile_business_owner_input",
        )
        profile_territory = st.selectbox(
            tr("My Territory (Parish)"), GRENADA_PARISHES,
            index=GRENADA_PARISHES.index((st.session_state.user_profile or {}).get("territory", GRENADA_PARISHES[0]))
            if (st.session_state.user_profile or {}).get("territory") in GRENADA_PARISHES else 0,
            key="profile_territory_input",
            help="Used to show you your closest IRD office.",
        )
        if st.button("💾 Save / Sign In", use_container_width=True):
            if not st.session_state.user_id:
                st.session_state.user_id = uuid.uuid4().hex[:10].upper()
            upsert_user_profile(
                st.session_state.user_id,
                st.session_state.user_name or "Anonymous",
                profile_email.strip(),
                profile_taxpayer_type,
                profile_is_business_owner,
                profile_territory,
            )
            st.session_state.user_profile = {
                "email": profile_email.strip(),
                "taxpayer_type": profile_taxpayer_type,
                "is_business_owner": "Yes" if profile_is_business_owner else "No",
                "territory": profile_territory,
            }
            st.session_state.pop("chat", None)  # refresh system prompt with new profile
            safe_toast("Profile saved!", icon="🪪")

    if st.session_state.user_id:
        st.success(f"{tr('Your Taxpayer ID')}: **{st.session_state.user_id}**")
        st.caption("Give this ID to IRD staff so they can find your info in their system.")
        my_office = find_office_for_parish((st.session_state.user_profile or {}).get("territory", GRENADA_PARISHES[0]))
        st.info(f"📍 Your closest office: **{my_office['name']}**\n\n{my_office['location']}")

    st.markdown("---")
    st.write(f"### 🌐 {tr('Language')}")
    ...
    SECTION_PAGES = {
        ...
        "📚 Help & Resources": [
            ...
            "🧮 Tax Estimators", "📅 Tax Deadline Calendar",
        ],
        "🔗 Links & Services": ["🔗 Useful Links & Services", "📰 Newsletter"],
        "🏢 Contact & Support": ["🏢 Offices", "🧑‍💼 Human Agent", "📅 Schedule Meeting", "🔒 Report an Issue"],
...
if WIDGET_MODE:
    st.markdown('<div class="widget-topbar">', unsafe_allow_html=True)
...
# -------------------------
# COMPACT FLOATING WIDGET: collapsed (avatar-only bubble) vs open (panel)
# -------------------------
if COMPACT_FLOATING and st.session_state.page == "💬 Chat with TESSA" and not st.session_state.compact_panel_open:
    # Collapsed: show ONLY the floating avatar (+ optional greeting
    # bubble) bottom-right. No model/API calls happen here - this is
    # pure UI state, exactly like opening/closing any other menu.
    if not st.session_state.compact_bubble_dismissed:
        st.markdown(
            '<div class="tessa-greeting-bubble">👋 Hi! I\'m TESSA.<br>How can I help you today?</div>',
            unsafe_allow_html=True,
        )
    st.markdown('<div class="tessa-float-avatar-wrap tessa-float-avatar-btn">', unsafe_allow_html=True)
    col_dismiss, col_open = st.columns([1, 3])
    with col_dismiss:
        if not st.session_state.compact_bubble_dismissed:
            if st.button("✕", key="compact_dismiss_greeting", help="Dismiss"):
                st.session_state.compact_bubble_dismissed = True
                st.rerun()
    with col_open:
        if st.button("💬", key="compact_open_avatar", help="Chat with TESSA"):
            st.session_state.compact_panel_open = True
            st.rerun()
    st.markdown('</div>', unsafe_allow_html=True)
    st.stop()

_compact_container_opened = False
if COMPACT_FLOATING and st.session_state.page == "💬 Chat with TESSA" and st.session_state.compact_panel_open:
    # Open: float the whole panel bottom-right. Deliberately enters the
    # container without a matching __exit__ - everything rendered for the
    # rest of this script run lands inside it, which is what lets a flat,
    # already-written top-to-bottom Streamlit script (this one) render
    # into a single fixed-position box without restructuring every page
    # into nested `with` blocks. Streamlit resets this cleanly on the
    # next rerun, so there's no leak.
    _float_ctx = st.container()
    _float_ctx.__enter__()
    st.markdown('<div class="tessa-float-marker"></div>', unsafe_allow_html=True)
    if st.button("✕ Close", key="compact_close_panel"):
        st.session_state.compact_panel_open = False
        st.rerun()
    _compact_container_opened = True

# -------------------------
# HEADER
# -------------------------
b64_avatar = get_avatar_b64()
avatar_mime = get_avatar_mime()
avatar_html = f'<img src="data:{avatar_mime};base64,{b64_avatar}" />'

_health = st.session_state.get("system_health", "online")
_status_label = (
    '🟢 System Online' if _health == "online"
    else '🟡 Running on Offline FAQ'
)
header_html = (
    ...
    f'&nbsp;·&nbsp; <span style="opacity:0.9;">{_status_label}</span></p>'
    f'</div></div>'
)
st.markdown(header_html, unsafe_allow_html=True)
if _health == "degraded":
    st.warning(
        "TESSA's live AI assistant is temporarily unavailable, so "
        "you're currently getting answers from the offline FAQ where "
        "possible. Staff have been notified. For anything urgent, use "
        "the 🧑‍💼 Human Agent tab.",
        icon="🟡",
    )
if not st.query_params.get("embed"):
    preview_toggle = st.toggle(
        "🔲 Compact mode (floating widget)", value=st.session_state.widget_preview,
        help="Shows TESSA as a floating bottom-right chat bubble, like the "
             "widget embedded on the IRD Grenada website, instead of the "
             "full-page layout.",
    )
    if preview_toggle != st.session_state.widget_preview:
        st.session_state.widget_preview = preview_toggle
        if preview_toggle:
            st.session_state.compact_panel_open = True
            st.session_state.compact_bubble_dismissed = False
        st.rerun()
...
if st.session_state.page == "💬 Chat with TESSA":
    ...
    # Collapsed into one popover in compact widget mode to avoid a tall,
    # scroll-inducing button row in a small corner bubble.
    if WIDGET_MODE:
        with st.popover("⋯ More", use_container_width=True):
            st.write(f"**Language:** {st.session_state.language} · **Tone:** {st.session_state.tone}")
            if st.button("📅 Book Appointment", use_container_width=True, key="w_book"):
                st.session_state["_open_meeting_dialog"] = True
            if st.button("🐞 Report a Bug", use_container_width=True, key="w_bug"):
                st.session_state["_open_bug_dialog"] = True
            if st.button("📄 Readiness Package", use_container_width=True, key="w_ready"):
                open_readiness_package()
            if st.session_state.messages:
                ...
                st.download_button(
                    ...
                    use_container_width=True, key="w_chatlog",
                )
    else:
    ...
    if not WIDGET_MODE:
        ...
        with st.popover("🗂️ Quick prompts", use_container_width=True):
            for bucket_name, prompts in PROMPT_BUCKETS.items():
                st.markdown(f"**{bucket_name}**")
                for p in prompts:
                    if st.button(p, key=f"wbucket_{bucket_name}_{p}", use_container_width=True):
                        st.session_state.pending_prompt = p
    ...
    try:
        chat_pane = st.container(height=(300 if WIDGET_MODE else 480))
    except TypeError:
        chat_pane = st.container()  # older Streamlit without `height=`

    with chat_pane:
        for i, msg in enumerate(st.session_state.messages):
            render_bubble(msg["role"], msg["content"], b64_avatar, avatar_mime)
            if msg["role"] == "assistant":
                speak_button(msg["content"], f"speak_{i}")
                safe_message_feedback(f"msgfb_{i}", msg["content"], st.session_state.user_name)

    # -------- Compact input bar: attach 📎, voice 🎤, and type - all in one row --------
    bar_col1, bar_col2, bar_col3 = st.columns([1, 1, 10])

    with bar_col1:
        with st.popover("📎", use_container_width=True, help="Upload a form (image/PDF) for TESSA to explain"):
            uploaded_form = st.file_uploader(
                "Upload a form and TESSA will walk you through it",
                type=["png", "jpg", "jpeg", "pdf"], label_visibility="collapsed",
            )
            if uploaded_form is not None and st.button("Explain this form", use_container_width=True):
                ...
                with st.spinner("TESSA is reviewing the form..."):
                    try:
                        ...
                        form_answer = friendly_api_error(e, where="Form Upload")
                st.session_state.messages.append(
    ...
    with bar_col2:
        with st.popover("🎤", use_container_width=True, help="Send a voice note to TESSA"):
            try:
                audio_value = st.audio_input("Record your question", label_visibility="collapsed")
            except Exception:
                audio_value = None
                st.caption("Voice notes need Streamlit 1.38+.")
            if audio_value is not None and st.button("Send voice note", use_container_width=True):
                if not check_rate_limit():
                    st.warning(RATE_LIMIT_MESSAGE)
                    st.stop()
                ...
                with st.spinner("TESSA is listening..."):
                    try:
                        ...
                        voice_answer = friendly_api_error(e, where="Voice Note")
    ...
    if prompt:
        ...
        if not classify_authority(prompt):
            ...
            response_slot.markdown(bubble_html("assistant", answer, avatar_for_bubble, avatar_mime), unsafe_allow_html=True)
        elif faq_answer:
            answer = faq_answer
            response_slot.markdown(bubble_html("assistant", answer, avatar_for_bubble, avatar_mime), unsafe_allow_html=True)
            ...
            response_slot.markdown(typing_indicator_html(avatar_for_bubble, avatar_mime), unsafe_allow_html=True)
            try:
                chunks = []
                # Retry opening the stream itself (transient errors only -
                # rate limit / brief outage). Once tokens start arriving we
                # don't retry mid-stream, since that would duplicate output
                # the user has already seen.
                stream = call_with_retries(st.session_state.chat.send_message_stream, prompt)
                for chunk in stream:
                    if chunk.text:
                        chunks.append(chunk.text)
                        response_slot.markdown(
                            bubble_html("assistant", "".join(chunks), avatar_for_bubble, avatar_mime),
                            unsafe_allow_html=True,
                        )
                st.session_state.system_health = "online"
                ...
                # than just showing an error. The raw exception is logged
                # for staff (Admin Dashboard -> Bugs), never shown as-is.
                offline_answer = fallback_faq_search(prompt)
                log_technical_error("Chat", e)
                kind = _error_kind(e)
                if kind in NON_TRANSIENT_ERROR_KINDS:
                    st.session_state.system_health = "degraded"
                if offline_answer:
                    ...
                    answer = friendly_api_error(e, where="Chat")
...
# =====================================================================
    ...
    st.subheader(tr("Frequently Asked Questions"))
    st.caption(f"{len(IRD_FAQ)} questions from the IRD Grenada knowledge base, grouped by topic.")

    search = st.text_input("🔍 Search all FAQs", placeholder="e.g. TIN, deadline, refund")
    if search:
        filtered = {
        ...
        if not filtered:
            ...
            for q, a in filtered.items():
                with st.expander(q):
                    st.markdown(a)
                    speak_button(a, f"faq_search_{q}")
    else:
        faq_categories = categorize_items(IRD_FAQ, FAQ_CATEGORY_KEYWORDS)
        cat_tabs = st.tabs(list(faq_categories.keys()))
        for tab, (cat_name, cat_items) in zip(cat_tabs, faq_categories.items()):
            with tab:
                for q, a in cat_items.items():
                    with st.expander(q):
                        st.markdown(a)
                        speak_button(a, f"faq_{cat_name}_{q}")
...
# =====================================================================
    ...
    st.subheader(tr("Tax Terms Glossary"))
    st.caption("Plain-language definitions of common IRD Grenada tax terms, grouped by topic.")

    glossary_search = st.text_input("🔍 Search all terms", placeholder="e.g. GCT, audit, penalty")
    if glossary_search:
        filtered_glossary = {
            term: definition for term, definition in GLOSSARY.items()
            if glossary_search.lower() in term.lower() or glossary_search.lower() in definition.lower()
        }
        ...
        glossary_categories = categorize_items(GLOSSARY, GLOSSARY_CATEGORY_KEYWORDS)
        glo_tabs = st.tabs(list(glossary_categories.keys()))
        for tab, (cat_name, cat_items) in zip(glo_tabs, glossary_categories.items()):
            with tab:
                for term, definition in sorted(cat_items.items()):
                    st.markdown(
                        f'<div class="faq-card"><strong>{term}</strong><br>{definition}</div>',
                        unsafe_allow_html=True,
                    )
...
# =====================================================================
    ...
    st.subheader(tr("IRD Offices Across Grenada"))
    st.caption("Select your territory to find your closest office, or browse all offices below.")

    st.markdown("#### 📍 Find My Closest Office")
    default_territory = (st.session_state.user_profile or {}).get("territory", GRENADA_PARISHES[0])
    picked_territory = st.selectbox(
        "My territory / parish", GRENADA_PARISHES,
        index=GRENADA_PARISHES.index(default_territory) if default_territory in GRENADA_PARISHES else 0,
    )
    closest = find_office_for_parish(picked_territory)
    st.success(f"Your closest office for **{picked_territory}** is **{closest['name']}**.")

    st.markdown("---")
    st.markdown("#### 🗺️ All Offices - Location & Map")
    for office in OFFICES:
        email_line = f"<br>✉️ {office['email']}" if office["email"] else ""
        highlight = " ⭐ (closest to you)" if office["name"] == closest["name"] else ""
        card_html = (
            f'<div class="office-card">'
            f'<h4>{office["name"]}{highlight}</h4>'
        ...
        with st.expander(f"🗺️ View {office['name']} on the map"):
            try:
                components.iframe(office_maps_embed_url(office), height=280)
            except Exception:
                pass
            safe_link_button("Open in Google Maps", office_maps_link(office), use_container_width=True)
...
# =====================================================================
    ...
    st.caption(
        ...
        "💬 Chat tab. Each guide can be saved or printed for offline use."
    )
    for doc_name, steps in DOCUMENT_GUIDES.items():
        with st.expander(doc_name):
            for i, step in enumerate(steps, start=1):
                st.markdown(f"**{i}.** {step}")
            pdf_bytes, pdf_ok = build_form_guide_pdf(doc_name, steps)
            safe_doc_name = re.sub(r"[^a-zA-Z0-9_-]", "_", doc_name)
            if pdf_ok:
                st.download_button(
                    "📥 Download / Print Guide (PDF)", data=pdf_bytes,
                    file_name=f"IRD_Grenada_{safe_doc_name}.pdf", mime="application/pdf",
                    use_container_width=True, key=f"formpdf_{safe_doc_name}",
                )
            else:
                st.download_button(
                    "📥 Download Guide (.txt)", data=build_form_guide_text(doc_name, steps),
                    file_name=f"IRD_Grenada_{safe_doc_name}.txt", mime="text/plain",
                    use_container_width=True, key=f"formtxt_{safe_doc_name}",
                )

# =====================================================================
# PAGE: TAX DEADLINE CALENDAR
# =====================================================================
elif st.session_state.page == "📅 Tax Deadline Calendar":
    st.subheader(tr("Tax Deadline Calendar"))
    st.caption(
        "Recurring deadline TYPES for quick reference. Exact yearly dates "
        "change, so always confirm the current year's date via the button "
        "below (live official search) before relying on it."
    )
    deadline_df = pd.DataFrame(TAX_DEADLINE_CALENDAR)
    deadline_df.columns = ["Frequency", "Item", "Typical Due Date", "Who It Applies To"]
    st.table(deadline_df)

    if st.button("🔎 Look up this year's exact deadlines", use_container_width=True):
        if not check_rate_limit():
            st.warning(RATE_LIMIT_MESSAGE)
        else:
            with st.spinner("Searching official sources..."):
                result_text, ok = deep_research(
                    "current official tax filing and payment deadlines for "
                    "this year for individuals and businesses in Grenada, "
                    "from the IRD Grenada website"
                )
            if ok:
                st.markdown(result_text)
            else:
...
# =====================================================================
# PAGE: REPORT AN ISSUE (security / hacked account / lost documents)
# =====================================================================
elif st.session_state.page == "🔒 Report an Issue":
    st.subheader(tr("Report an Issue"))
    st.caption(
        "Use this if your G-TAX/portal account may have been hacked, you "
        "spot suspicious activity, lost an official document, or suspect a "
        "scam. This creates a report IRD staff can review in their system."
    )
    st.warning(
        "🚨 If this is a live emergency (e.g. money is actively being "
        "stolen right now), also call the IRD directly or your bank "
        "immediately - don't rely on a form alone."
    )
    with st.form("security_report_form", clear_on_submit=True):
        sec_name = st.text_input("Your name", value=st.session_state.user_name, placeholder="e.g. Alicia")
        sec_contact = st.text_input("Best way to reach you", placeholder="Phone or email")
        sec_issue_type = st.selectbox("What happened?", SECURITY_ISSUE_TYPES)
        sec_description = st.text_area(
            "Describe what happened (don't include passwords or full card/ID numbers)",
            placeholder="e.g. I received a text saying my account was suspended and asking for my TIN...",
        )
        sec_submit = st.form_submit_button("🚨 Submit Report", use_container_width=True)

        if sec_submit:
            if not sec_contact.strip() or not sec_description.strip():
                st.warning("Please fill in your contact info and a description.")
            else:
                clean_description, _ = redact_pii(sec_description.strip())
                save_security_report(
                    st.session_state.user_id, sec_name.strip(), sec_contact.strip(),
                    sec_issue_type, clean_description,
                )
                safe_toast("Report submitted!", icon="🚨")
                st.success(
                    "✅ Your report has been logged and will be reviewed by "
                    "IRD staff. For anything urgent, also use the "
                    "🧑‍💼 Human Agent tab to reach someone directly."
                )
...
# =====================================================================
    ...
    st.subheader("Admin Dashboard - Staff Only")

    if not ADMIN_PASSWORD:
        st.error(
            "⚠️ No staff password is configured yet, so this dashboard is "
            "locked to everyone. Set `ADMIN_PASSWORD` in "
            "`.streamlit/secrets.toml` (or as an environment variable) to "
            "enable staff access."
        )
        st.stop()
    ...
    if not st.session_state.admin_authenticated:
        st.caption("This area is restricted to IRD Grenada staff.")
        ...
        if login_submit:
            if entered_password == ADMIN_PASSWORD:
                ...
                safe_toast("Welcome, staff!", icon="🔓")
                st.rerun()
            else:
                st.error("Incorrect password.")
        st.stop()

    col_admin_a, col_admin_b = st.columns([4, 1])
    with col_admin_a:
        st.caption("Read-only view of pending requests, reports, and recent activity.")
    with col_admin_b:
    ...
    tab_labels = [
        "👤 Users", "🔒 Security Reports", "📞 Callback Requests", "📅 Meetings",
        "🐞 Bugs", "⭐ Feedback Summary", "👍 Message Ratings", "📜 Recent Interactions",
    ]
    admin_tabs = st.tabs(tab_labels)

    with admin_tabs[0]:
        st.caption("Search by the unique Taxpayer ID the user was given at sign-in.")
        lookup_id = st.text_input("🔍 Look up a Taxpayer ID", placeholder="e.g. A1B2C3D4E5")
        users_df = load_users_df()
        if lookup_id.strip():
            match = users_df[users_df["user_id"].str.upper() == lookup_id.strip().upper()] if not users_df.empty else users_df
            if match.empty:
                st.warning("No user found with that ID.")
            else:
                st.dataframe(match, use_container_width=True)
        else:
            st.metric("Total signed-in users", len(users_df))
            st.dataframe(users_df, use_container_width=True) if not users_df.empty else st.info("No users signed in yet.")

    with admin_tabs[1]:
        df = load_security_reports_df()
        st.metric("Total security/incident reports", len(df))
        st.dataframe(df, use_container_width=True) if not df.empty else st.info("No security reports yet.")
